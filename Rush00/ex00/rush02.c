...
/*   Updated: 2021/03/28 14:53:14 by bna              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	when_to_print(int width, int height, int x_position, int y_position)
{
	if (0 == y_position)
	{
		how_to_print_corner('A', 'A', x_position, width);
		how_to_print_mid('B', x_position, width);
	}
	else if (height - 1 == y_position)
	{
		how_to_print_corner('C', 'C', x_position, width);
		how_to_print_mid('B', x_position, width);
	}
	else
	{
		how_to_print_mid(' ', x_position, width);
		how_to_print_mid_first_last('B', x_position, width);
	}
}

/*
**A procedure using all of the procedures in rush02
*/
